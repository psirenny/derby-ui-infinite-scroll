...
exports._pageDown = function (e, el, dom, model) {
  var _this = this
    ...
    , getter = model.get('options.subscribe') ? 'subscribe' : 'fetch';

  this.root[getter](target, function (err, results) {
    ...
    model.set('pageNumbers', model.get('pageNumbers').concat([n]));
...
exports._pageUp = function (e, el, dom, model) {
  ...
  this.root[getter](target, function (err, results) {
    ...
    model.ref('pages.' + n + '.ref', results);
    model.fn('pages.' + n + '.items', results, function (results) { return reverse ? results.reverse() : results; });
    model.set('pageNumbers', [n].concat(model.get('pageNumbers')));
    stub.set('height', Math.max(0, stub.get('height') - (el.scrollHeight - scrollHeight)));
    stub.set('length', Math.max(0, stub.get('length') - results.get().length));
    if (model.get('options.maxPages')) _this.trimPages(e, el, dom, model);
  });
...
exports.init = function (model) {
  var options = {
    ...
    , subscribe: model.get('subscribe') || true
    , throttle: model.get('throttle') || 500
...
exports.create = function (model, dom) {
  ...
  scope.fn('topPageNumber', scope.at('pageNumbers'), function (pageNumbers) { return pageNumbers ? pageNumbers[0] : null; });
  ...
  stubs.fn('bottom.cssDisplay', stubs.at('bottom.height'), function (height) { return height ? '' : 'none'; });
  stubs.fn('top.cssDisplay', stubs.at('top.height'), function (height) { return height ? '' : 'none'; });
